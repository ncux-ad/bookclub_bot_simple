...
### 📚 Управление книгами
9. [x] **Команда `/addbook`** - Добавление новых книг через FSM
   - [x] FSM для пошагового добавления книг
   - [x] Валидация названия, автора, описания
   - [x] Поддержка файлов EPUB, FB2, MOBI
   - [x] Проверка дубликатов книг
   - [x] Интерактивные кнопки отмены и пропуска
   - [x] Интеграция с системой ролей

10. [x] **Команда `/uploadbook`** - Загрузка книг из ZIP-архивов FB2
    - [x] Обработка ZIP-архивов с FB2 файлами
    - [x] Парсинг метаданных FB2 (автор, название, год, жанр)
    - [x] Конвертация FB2 → EPUB → MOBI
    - [x] Загрузка файлов в Telegram для шаринга
    - [x] Расширенная база данных с полями для ссылок
    - [x] Автоматическая очистка временных файлов
    - [x] **Мультиплатформенность** - поддержка Windows и Ubuntu
    - [x] **Портативный Calibre** - встроенная поддержка портативной версии
    - [x] **Автоопределение Calibre** - поиск в портативной и системной установке
    - [x] **Обработка кодировок** - поддержка разных кодировок для Windows
    - [x] **Исправлена загрузка в Telegram** - использование FSInputFile для корректной передачи файлов
    - [x] **Команда `/checkcalibre`** - проверка установки и инструкции

11. [x] **Система библиотеки книг** - Полноценная работа с библиотекой
    - [x] **Команда `/library`** - Просмотр всех книг в библиотеке
    - [x] **Карточки книг** - Детальная информация о каждой книге
    - [x] **Скачивание в разных форматах** - FB2, EPUB, MOBI
    - [x] **Поиск книг** - По названию, автору, описанию
    - [x] **Ссылки на онлайн каталоги** - Яндекс.Книги, ЛитРес, аудиоформат
    - [x] **Команда `/editlinks`** - Редактирование ссылок для админов
    - [x] **FSM для редактирования ссылок** - Пошаговое редактирование
    - [x] **Исправлена ошибка BUTTON_DATA_INVALID** - Использование MD5 хеша названий книг для callback_data
    - [x] **Исправлена отправка файлов** - Использование FSInputFile для корректной отправки документов

12. [ ] **Команда `/editbook <title>`** - Редактирование информации о книге
13. [ ] **Команда `/delbook <title>`** - Удаление книг из библиотеки
...
### 📋 Отложенные задачи
77. [ ] **Команда `/massban <criteria>`** - Массовая блокировка по критериям (отложено - неактуально)